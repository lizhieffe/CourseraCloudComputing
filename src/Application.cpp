...
void Application::mp1Run() {
	...
	for( i = 0; i <= par->EN_GPSZ-1; i++) {
         ...
         * Only do for the nodes that is already started and not failed
		 */
		if( par->getcurrtime() > (int)(par->STEP_RATE*i) && !(mp1[i]->getMemberNode()->bFailed) ) {
            ...
            /*
             * send Heart Beating to part of the peers
             */
            vector<Address> addrToSendHeartBeating;
            for (int j = 0; j < par->EN_GPSZ - 1; ++j)
                if (i != j && i % 2 == j % 2)
                    addrToSendHeartBeating.push_back(mp1[j]->getMemberNode()->addr);
            mp1[i]->sendHeartBeating(par->getcurrtime(), addrToSendHeartBeating);
		}
